<x-app-layout>
    <x-slot name="header">
        <h2 class="text-2xl font-semibold text-gray-900">Ajouter une catégorie</h2>
    </x-slot>
            ...
            <div class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm sm:p-8">
                <form method="POST" action="{{ route('admin.categories.store') }}" class="space-y-6">
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="5" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-teal-600 focus:ring-teal-600">{{ old('description') }}</textarea>
                        <x-input-error class="mt-2" :messages="$errors->get('description')" />
                    </div>
                    <div class="flex gap-3">
                        <x-primary-button>Créer la catégorie</x-primary-button>
